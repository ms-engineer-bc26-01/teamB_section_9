...
| UI-005 | 提案生成中画面       | `/outfits/loading`       | 認証        | AI がコーデを生成している間の待機状態を表示する                 | ローディング表示、「コーデを提案中です」などのメッセージ                           | 一定時間後にコーデ詳細画面へ移動               | 現状は画面遷移のみ。将来的に `POST /outfits/suggest` と連携予定                          | 実装中    |
