...
import {
  createClothing,
  createClothingUploadUrl,
} from "@/features/clothes/api";
import type {
...
const supportedImageTypes: ClothingUploadUrlRequest["content_type"][] = [
  "image/jpeg",
  "image/png",
  "image/webp",
];
  ...
  const handlePhotoChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      !supportedImageTypes.includes(
        file.type as ClothingUploadUrlRequest["content_type"],
      )
    ) {
    ...
    try {
      const { upload_url, image_url } = await createClothingUploadUrl({
        filename: file.name,
        content_type: file.type as ClothingUploadUrlRequest["content_type"],
      });
        ...
        <Card className="border-[#E8DED4] bg-white shadow-sm">
          ...
          <CardContent className="space-y-3">
            ...
            <button
              ...
              className="flex aspect-[4/3] w-full cursor-pointer flex-col items-center justify-center gap-3 overflow-hidden rounded-2xl border border-dashed border-[#D8CABE] bg-[#FAF8F5] text-sm text-[#6B4F3A] disabled:cursor-not-allowed"
        ...
        <Card className="border-[#E8DED4] bg-white shadow-sm">
          ...
          <CardContent className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                {tpoTags.map((tag) => {
                  const isSelected = selectedTpoTags.includes(tag.value);
