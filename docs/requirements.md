...
| 期日 | 内容 | 担当 |
|---|---|---|
| 2026-05-25 | 47 都道府県の代表 1 地点（県庁所在地）で完成 | D（バックサブ）、レビュー: 全員 |
| 2026-06-01 | 高・中優先度に 2〜3 地点を追加（計 60〜80 件） | D |
...
チームミーティングで決定し、本ファイルを更新すること。

| 論点 | 推奨案 | 決定期日 |
|---|---|---|
| 地域マスタの細分化スケジュール（高・中優先度の追加タイミング） | 4.2 節のスケジュール表通りに段階対応 | 2026-06-01（D に確認） |
| 本番ドメインの命名 | MVP 凍結（6/19）後に着手時に決定 | MVP 凍結後 |
| オンボーディングのスキップ可否 | 必須にして `default_region_code` 未設定を発生させない | 実装前（S1 開始まで） |
| 旅行先など一時的な地域変更の履歴管理 | その都度入力（`region_code` パラメータ）。履歴は持たない | 実装前（S1 開始まで） |
| 地域マスタの追加要望が出た場合の運用 | Issue 起票 → PR 対応のルール化 | S1 開始前 |
