...
> 📱 つまずきやすいポイントまで網羅した手順書は **[スマホ実機検証マニュアル](./スマホ実機検証マニュアル.md)** にまとめています。
> 初めての人・「認証状態を確認中…」で固まる人はそちらを参照してください。
...
2. **フロントの API 向き先をホスト IP に変更**
   ...
   変更後はフロントを**リビルド**する（本プロジェクトはソースをイメージに `COPY` する構成のため、
   `.env` 変更や `git pull` はリビルドしないと反映されない）：
...
3. **CORS は編集不要（開発時のみ自動許可）**
   ...
   また Next.js の dev リソースの cross-origin ブロック対策（`allowedDevOrigins`）も
   `frontend/next.config.ts` に設定済みのため、各自の編集は不要です。
   （未設定だと画面が「認証状態を確認中…」で固まる。詳細はマニュアル参照。）
...
5. **つながらない場合**
   ...
   - Windows ファイアウォールで **inbound の 3000 / 8000** を許可。ネットワークが「パブリック」だと
     初回ダイアログが出ないことがあるので、管理者 PowerShell で明示的に許可する（[マニュアル手順 4](./スマホ実機検証マニュアル.md#4-windows-ファイアウォールで-3000--8000-を許可する)）。
   - `NEXT_PUBLIC_API_BASE_URL` を `localhost` のままにしていないか確認（スマホからは到達不可）。
   - 画面は出るが**「認証状態を確認中…」で固まる**場合は、フロントの**リビルド漏れ**を疑う
     （`docker compose up -d --build frontend`）。背景は[マニュアル手順 7](./スマホ実機検証マニュアル.md#7-補足認証状態を確認中で固まる現象について重要な背景)。
